...
// ============================================================================
// <CustomCursor> — a soft glowing blue dot + thin lagging hairline ring that
// trails the pointer, so the cursor feels ALIVE on the editorial broadsheet.
// Ported verbatim from apps/crash; mounted GLOBALLY by App.tsx's Shell on the
// whole signed-in wallet.
// ----------------------------------------------------------------------------
// The OS cursor is hidden via the `cursor: none` rule in tokens-journal.css
// (scoped under `.journal`, fine pointers only); this draws the dot + ring.
// Touch/coarse-pointer devices have no hover cursor — we detect that and render
// nothing (CSS hides the layers too). Pure transform animation on two layers
// (a crisp dot + a lagging ring) via rAF; no React re-render per frame, no deps.
// ============================================================================
export function CustomCursor() {
  ...
  // The live signed-in wallet styles these layers via the `.j-cursor-*` rules in
  // tokens-journal.css (loaded globally through src/styles/index.css).
  return (
    <>
      <div ref={ring_ref} className="j-cursor-ring" aria-hidden="true" />
      <div ref={dot_ref} className="j-cursor-dot" aria-hidden="true" />
    </>
  );
}
