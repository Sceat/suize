// ============================================================================
// Suize Agents — config. SINGLE SOURCE OF TRUTH for network + on-chain ids lives
// in @suize/shared (LOCKED #15); this file only resolves env and re-exports.
// Mirrors apps/pay/src/config.ts exactly (env shape, API_BASE resolution).
// ============================================================================
...
// Re-export the address validator from the single source of truth so this app's
// `import … from './config'` call sites are untouched.
export { SUI_ADDRESS_RE }

// The app's base path — '' (vite `base: '/'`, trailing slash trimmed): STANDALONE
// at agents.suize.io.
export const BASE_PATH: string = import.meta.env.BASE_URL.replace(/\/+$/, '')

// Enoki seedless Google zkLogin — agents.suize.io runs its OWN sign-in so a
// visitor can bid on an ad slot without leaving the page. Same shared OAuth
// client the wallet/pay use; the redirect uri `${origin}/enoki` is whitelisted
// on it. Empty -> Enoki registration is skipped and `sign_in_with_google`
// no-ops (the Connect-a-wallet path stays).
...
// override first; dev falls back to the local backend (services/backend .env
// PORT=8099), prod builds to the live API.
...
// Explorer link for digests shown in the feed.
export const SUIVISION_TX = (digest: string) =>
  `https://${NETWORK === 'mainnet' ? '' : NETWORK + '.'}suivision.xyz/txblock/${digest}`

// Explorer link for an address (the merchant/holder rows).
export const SUIVISION_ADDR = (address: string) =>
  `https://${NETWORK === 'mainnet' ? '' : NETWORK + '.'}suivision.xyz/account/${address}`
