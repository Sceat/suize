...
- A payment-less `POST /deploy` answers **402** with the **x402 V2 `PaymentRequired` body + `PAYMENT-REQUIRED` header** (price discovery is public; minted via `@suize/pay`'s `mintPaymentRequired`, facilitator/buildUrl pointed at this process's own origin). The 402's `error` carries the deploy rider: **whoever pays owns the site**.
- The agent builds the gasless `send_funds` payment (`POST /build` with the challenge's `accepts[0].extra.outputs`, or its own PTB), signs the payment **ITSELF** (TWO DOORS, ONE WIRE: a **Sui-aware** agent signs with its **own Sui key**; a **Suize** agent signs with its zkLogin session via `@suize/mcp`), and retries multipart with the b64 `PaymentPayload` in the **`X-PAYMENT`** header — **the sole authorization, NO separate deploy-auth nonce/signature** (nonce-free since 2026-06-14).
- `gateDeployPayment` (in-process — no HTTP loopback): unb64 the payload → its `accepted` must deep-equal OUR single-output terms → **recoverPayer → the on-chain `owner`** (whoever pays, owns; no payer==owner compare — the payer simply IS the owner) → `doVerify` (simulate-only, exact split) → `settleDeployPayment` (`doSettle` — broadcast keyless, idempotent by digest) settles it. **The ordering: VERIFY → SETTLE → Walrus upload → `create_site`** — settlement lands BEFORE any Walrus write, so an unsettled payment never burns WAL. `create_site` records the settled digest in the on-chain **`SiteDigestRegistry`** (one-site-per-payment; a duplicate aborts **`EDigestUsed` → 409**, the multi-replica-safe consume guard — no in-process map). A retry with the same `X-PAYMENT` re-settles idempotently and re-mints only if the prior mint didn't land. No payment → no deploy. E2E: `deploy.402.e2e.ts` + `deploy.paid.e2e.ts`.
