...
import { SITES, epochsRemaining, shortDigest } from '../data'
import type { DeploySite, Preview } from '../types'

// The front page — the gallery IS the news. Each card is a DeploySite; the epoch
// countdown is derived live from the Walrus epoch clock. Placeholder rows today;
// T-005b: live chain data — swap SITES for on-chain SiteCreated events, nothing
// below changes.
  ...
  return (
    <article className="story story--lead">
      ...
      <div className="leadbar">
        <span className="epoch epoch--perm">
          <b>{rem === 'permanent' ? 'Permanent' : rem}</b> · funded pool
        </span>
        <span className="dots" />
        {site.receiptDigest && (
          <a className="receipt" href="#">
