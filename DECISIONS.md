...
2026-07-18 · Demo architecture page: owner picked variant C "scroll journey" (six-beat spine, giant 402, monument pillars) over A transit-map and B poster-bands, with a mechanism pass: every beat gains a visible one-line HOW (MCP signs via the user's Sui CLI, facilitator = plain standard x402 keyless, cache = edge copies with Walrus as truth, Site object shared with owner field = the payer) and the Cloudflare-down state must answer HOW sites stay reachable (any Walrus aggregator by content id, or self-deployed open-source worker) · "it does this, but how? magic?" = the standing bar for all demo material.
2026-07-18 · PRICING: deploy $0.25 per site-month (was $0.10), sealed keeps 2x = $0.50, domains unchanged $19.99/yr · driven by measured unit economics (one-time Walrus write ~0.25 WAL ~= $0.11 + ~$0.06 gas per store made the $0.10 first month break-even-to-negative); at $0.25 every deploy is margin-positive from month one, extends stay nearly pure margin. One-constant change in @suize/shared by law; @suize/mcp bumps to 0.3.1 (bundled guard).
