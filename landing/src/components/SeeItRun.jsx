...
const STATES = [
  ...
  },
  {
    ...
    body:
      '"Top 5 memecoins worth caring about" is judgment, not lookup. The agent passes consensus: 5. Five interpretation passes fire in parallel. We let them disagree.',
  },
  {
    eyebrow: '· 03 — answer',
    title: 'Convergence. The number that tells you what to trust.',
    body:
      'Five passes aggregate into one ranking with convergence 0.78. The agent acts knowing exactly how much epistemic weight the answer carries.',
  },
]
...
function ScrollTerminal ({ stage, localT }) {
  ...
  const semanticIntent = 'sui.ask("top 5 memecoins worth caring about", consensus: 5)'
...
{`pass 1 → [BLUB, PUPS, AXOL, GROK, DOGS]
pass 2 → [BLUB, PUPS, GROK, AXOL, MOON]
pass 3 → [BLUB, AXOL, PUPS, GROK, MEME]
pass 4 → running...
pass 5 → running...`}
            </pre>
          </>
        ...
        {stage === 2 && (
          <>
            <div className="flex items-center gap-2 text-[color:var(--color-ink-mute)] text-[11px] sm:text-[12px]">
              <span className="text-[color:var(--color-sui-bright)] shrink-0">✓</span>
              <span>5/5 aggregated · convergence 0.78</span>
            </div>
            <div className="flex flex-col gap-2 mt-1">
              <div className="flex items-start gap-2">
                <span className="text-[color:var(--color-sui-bright)] shrink-0">→</span>
                <span className="text-[color:var(--color-ink)] break-words">
                  BLUB, PUPS, AXOL, GROK, MOON
                </span>
              </div>
              <pre className="ml-5 text-[11px] sm:text-[12px] text-[color:var(--color-ink-dim)] whitespace-pre-wrap break-words">
{`{ ranked: ["BLUB","PUPS","AXOL","GROK","MOON"], convergence: 0.78, consensus_n: 5 }`}
              </pre>
            </div>
          </>
        )}
