...
> **Caveat (in-flight hardening — read before pitching this as absolute).** The budget gate and the `idle/deployed` vault custody hold today, but the **swap and NAVI-withdraw legs are being hardened** (pinned pool + asset-bound tickets). **Until the wallet republishes, treat those two legs as not-yet-fully-VM-caged** — on those specific paths the "impossible to construct" / "funds never leave Move custody" guarantee is the target shape, not yet the shipped one. The mandate gate + vault round-trip on the NAVI-supply and `idle/deployed` paths are caged today; the swap leg and the NAVI-*withdraw* leg are the two exceptions in flight.
...
Per-user `Vault<T>` (`idle` + `deployed` `Balance<T>`, **never pooled**). Owner deposit / idle-withdraw; the **tight cage** `agent_consume` wraps `consume_budget` so the **budget debit is ATOMIC with the real coin movement** (one PTB, no way to move coins without debiting the mandate; funds never leave Move custody — the adapters below specialize the inner move with the real protocol call). **In flight:** the "funds never leave Move custody" invariant holds for the `idle/deployed` and NAVI-supply paths today, but the **swap and NAVI-withdraw legs are being hardened** (pinned pool + asset-bound tickets); until the wallet republishes, treat those two legs as not-yet-fully-caged (see the §3 caveat).
...
- **Positioning:** **Responsible Degen** — *"permission to gamble safely."* Not a beat-the-market robo-advisor (never claim alpha), not a custodial yield farm (funds never leave Move custody — *with the swap + NAVI-withdraw legs still being hardened, §3 caveat; don't pitch this as absolute until the wallet republishes*). Dedicate play money; the chain caps the loss at the sandbox wall.
