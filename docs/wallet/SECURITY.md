...
The agent's signing key **cannot construct a transaction that exceeds these bounds** — the Move VM **aborts the tx on-chain** if it tries. This is the structural difference from a confirm-button wallet: we don't need a human tap because the chain is the leash. (The shipped custody-tightness looseness is **three legs in flight** — the long-standing NAVI supply-leg destination, §1.6, plus the DeepBook-swap and NAVI-withdraw legs being hardened, §1.3 — none of which touch the budget/scope/expiry enforcement, which is enforced on every leg today.)
...
> **STATUS (in-flight hardening — overrides the per-leg "tight" wording below until the wallet republishes).** An audit confirmed the custody-tightness of **two legs is not yet fully shipped**: the **DeepBook spot swap leg** and the **NAVI-withdraw leg** are being hardened (**pinned pool + asset-bound tickets**) so the redeemed/returned coins are forced back into vault custody with no agent-redirectable span. **Until the wallet republishes, treat both of those legs as not-yet-fully-VM-caged on the custody dimension** — alongside the long-standing NAVI-*supply* caveat (§1.6). What still holds today on every leg: the **amount/scope/expiry gate** (`consume_budget`) and **total revocation**; what's in flight is the per-leg *destination* tightness for swap + NAVI-withdraw. The sub-bullets below describe the **target** shape; don't pitch the swap/withdraw legs as already custody-tight.
...
- **DEGEN / DeepBook spot — `swap.move` (target: tight, VM-enforced — hardening in flight):** the agent splits the input out of the `SwapVault`, threads the DEEP fee from the vault's own pot, calls `pool::swap_exact_*` (`min_out`-gated), and **re-joins all three returned coins** (leftover input, output, leftover DEEP) straight back into the vault. The target is that **no `Coin` is ever returned to the caller** — the agent has nothing to redirect through the entire round-trip. **Not yet fully shipped:** the leg is being hardened with a **pinned pool + asset-bound tickets** so the returned coins can't be redirected; until the wallet republishes, treat this leg as not-yet-fully-custody-caged (the amount/scope/expiry gate still holds). Spot SUI↔USDC is otherwise a clean Coin-in/Coin-out path with no pause risk.
- **SAFE / NAVI — `navi.move` (mixed, FLAGGED):** the vault custodies NAVI's `AccountCap`, so the lending position belongs to the vault. The **withdraw** leg (scope 1) is **being hardened to tight** — an asset-bound `WithdrawTicket` **hot potato** (no abilities) is intended to force the redeemed coin back into custody (the tx cannot complete otherwise); **until the wallet republishes, treat the withdraw leg as not-yet-fully-custody-caged** alongside the supply leg. The **supply** leg (scope 0) is the long-standing **looser** leg — see §1.6.
...
- **The NAVI withdraw leg is being hardened to close the loop tightly** via an asset-bound `WithdrawTicket` hot potato (no `drop`/`store` → the tx can't complete without re-absorbing into the vault); until the wallet republishes, the withdraw leg's custody-tightness is in flight (§1.3), though its amount/scope gate holds today.
...
- **Our response:** The exploit our architecture answers most directly. The agent key is **not privileged** — it is scoped by the mandate; a stolen key cannot drain everything because budget/scope/expiry bound it and the owner can revoke, on **both** MVP tiers (no un-cageable margin manager exists). **Turnkey enclave** (prod) avoids a raw operator key on a hot server. *(Caveat: §1.3/§1.6 — the NAVI supply leg's released-coin destination isn't VM-enforced, and the DeepBook-swap + NAVI-withdraw legs are still being hardened to tight, so a stolen key + a malicious PTB could misroute one mandate-sized coin on a not-yet-hardened leg; the budget still caps the size on every leg.)*
...
|---|---|---|---|
| 1 | **Custody-tightness of three legs (§1.3, §1.6)** | The amount/scope/expiry gate holds on every leg, but the *coin-destination* tightness of three legs is in flight: the long-standing NAVI **supply** leg (NAVI isn't importable, §1.6) plus the **DeepBook-swap** and **NAVI-withdraw** legs being hardened (pinned pool + asset-bound tickets, §1.3). A malicious PTB could in principle misroute a mandate-sized coin on a not-yet-hardened leg. | Budget caps the size; off-chain allow-list on the agent's PTB; **swap + withdraw legs hardening to tight before the wallet republishes**; supply-leg tighten via localnet integration (assert the coin lands at NAVI) or when NAVI becomes importable. |
...
| 6 | **Agent host compromise (pre-enclave)** | Until Turnkey is the signing path, a scoped agent key may sit on a server. | Authority is mandate-bounded + revocable even if stolen (both tiers); the extra exposure is the not-yet-hardened custody legs (§1.3 swap + NAVI-withdraw, §1.6 NAVI-supply destination). **Move to enclave before scaling.** |
...
4. **Three legs' coin-destination is not yet fully VM-caged (§1.3, §1.6).** Amount + scope are caged on every leg, but custody-tightness is in flight on three: the long-standing NAVI **supply** leg (until NAVI is importable or a localnet integration asserts it) plus the **DeepBook-swap** and **NAVI-withdraw** legs being hardened (pinned pool + asset-bound tickets) before the wallet republishes. Revoke is total on every leg.
