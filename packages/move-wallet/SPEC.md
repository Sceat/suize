# move-wallet — The On-Chain Rail Contract (canonical)

> The single source of truth for Suize's on-chain payment rail: the `Account<USDC>` object, the `Subscription` child, the four payment verbs, the fee mechanics, the abort-code contract, and the lifecycle (deposit / withdraw / cancel / kill). The global picture — what Suize is, the two-primitive law, custody posture, the off-chain surface (the PAY wallet app; the optional dev-side MCP), network plan — lives in the root `CLAUDE.md`; this file owns only the chain. House style: state each fact once, reference don't redeclare.
>
> **Package:** `suize` (`edition = "2024"`), module `suize::account` at `sources/account.move`. Framework-only build (`sui move build` + `sui move test` green; **28 `account` unit tests**, 98 total across the package). **Unpublished** — `PACKAGE_IDS.ACCOUNT = 0x0` in `@suize/shared`; **publishing to mainnet is the v1 gate.** The directory is named `move-wallet` for historical reasons but this is **THE RAIL**, not "the wallet" (the wallet app is just one consumer of it). A rename to `move-account` / `move-rail` is a future cleanup — do NOT rename now (it would churn package ids, imports, and `@suize/shared`).
>
...
- **`Account` is a SHARED object.** Rationale — *shared ONLY because the relayer must deduct subscriptions without the owner signing*: `charge_subscription` (the permissionless recurring rail) and `pay` (the raw-payer facilitator) and `deposit` (anyone tops up) must be callable in transactions the owner does not co-sign. If subscriptions did not exist, this could be an owned object. Owner-only paths (`spend` / `charge` / `withdraw` / sub create+cancel) assert `sender == owner` regardless of the object being shared.
...
- **Rate resolution:** `overrides[merchant]` if present, else `default_fee_bps`. Overrides are typically per-merchant **discounts** (e.g. 50 bps = 0.5%), but any rate `≤ 10_000` (100%) is allowed. The relevant merchant per verb: **`charge`** → its `merchant` param; **`pay`** → `merchant_account.owner`; **`charge_subscription`** → the subscription's FIXED `payee`.
...
|---|---|---|---|---|---|
| ① | `spend(account, amount, payee, memo, clock, ctx)` | OWNER-only | **FREE** | yes | **exists** |
| ② | `charge(account, config, merchant, amount, memo, clock, ctx)` | OWNER-only | **rate@merchant** | yes (payer) | **exists** |
| ③ | `charge_subscription(account, config, sub_key, amount, clock, ctx)` | PERMISSIONLESS, terms-gated | **rate@payee** | yes | **exists** |
| ④ | `pay(merchant_account, config, payment: Coin<T>, memo, clock, ctx)` | PERMISSIONLESS | **rate@owner** | **no payer Account** | **exists** |
...
`pay<T>(merchant_account: &Account<T>, config: &RailConfig, payment: Coin<T>, memo: vector<u8>, clock: &Clock, ctx: &mut TxContext)`

The open facilitator: a one-off charge from ANY raw payer with a `Coin<T>` in hand — **NO payer Suize Account required**. This is the door for external 402 / AP2 agents that have USDC but no funded Account. **PERMISSIONLESS** (no owner gate — the payer's signature over the `payment: Coin<T>` input IS the authorization; you can only pay with a coin you own). There is **no `EInsufficientBalance` check** — the coin's value IS the amount, so you cannot over-spend a coin you handed in. **FEE SOURCE (refactor — see §4):** the rate + recipient are read from the shared **`RailConfig`**, resolved against the merchant being paid (`merchant_account.owner`) — NOT from the merchant's own Account (a merchant can't zero their own rate) and NOT a constant. The `merchant_account` is still `&`-borrowed (read-only) only to learn `owner`. `net` is **transferred** to that owner and `fee` to `config.fee_recipient`, both as fresh `Coin<T>`s (same payout primitive as `charge` — transfer, never a deposit-into-Account). Emits **`Paid`** with the fee visible (no `account_id` — the payer has no Account). Carves the fee off the handed-in coin with `coin::split`.
...
- **`fee_bps_for(config, merchant) -> u16`** resolves the rate. The merchant is: verb ② `merchant` param · verb ③ the sub's FIXED `payee` · verb ④ `merchant_account.owner`.
...
- **`pay` (④)** cannot use `split_and_pay` (it operates on a raw `Coin<T>`, not the balance); it performs the **same arithmetic** against the handed-in coin via `coin::split`, with the rate from `fee_bps_for(config, merchant_account.owner)` and the fee → `config.fee_recipient`.
...
- **`Charged`** = same shape **plus `sub_key`**, and it **does** carry the real 2% fee.
...
- **`Paid`** (verb ④ `pay`) carries `{ payer, merchant, gross, fee, net, memo, timestamp, decision_hash, walrus_blob_id }` (no `account_id` — there is no payer Account in that path; `payer` = `ctx.sender()`).

---

## 8. Tests & build status

- **28 unit tests** pass for `suize::account` (`sui move test`; **98 total** across the package incl. the legacy cage suites); `sui move build` green; framework-only (no vendored protocol deps).
...
- The two CHARGE-via-config verbs are tested run by an **arbitrary non-owner sender** where applicable (`pay` permissionless, no payer Account) + the merchant Account balance proven untouched (paid by transfer, not deposit).
- **Unpublished** (`PACKAGE_IDS.ACCOUNT = 0x0`). Publishing to **mainnet** is the v1 gate; the legacy cage tests (`mandate`/`vault`/`swap`/`navi`, 65/65) describe the OLD, dead product — do NOT read "65/65 green" as "the rail is built." See `CLAUDE.md` for the cage-retirement note and the mainnet sequence.

---

**Refactor note (fee policy moved to `RailConfig`):** previously every CHARGE verb read `fee_bps` + `fee_recipient` off an `Account` — verb ④ `pay` read them off the *merchant's* Account. That was unsafe for Suize's revenue: a merchant could set their own `fee_bps = 0` (pay Suize nothing), and there was no way to grant a specific merchant a lower rate. The rate is now **Suize's**, in one shared `RailConfig` (cap-gated), resolved **per-merchant** (override or default). `pay` still `&`-borrows the merchant Account, but ONLY to learn `merchant_account.owner` (the merchant address) — the rate + recipient come from `config`. Trade-off unchanged: `pay` still requires the merchant to have a Suize Account (now only to resolve its `owner`), which is fine — a "Suize merchant" is by definition someone with an Account on the rail. Downstream: every CHARGE PTB (`charge` / `charge_subscription` / `pay`) must now pass the shared `RailConfig` object id, and `@suize/shared` needs a `RAIL_CONFIG` shared-object slot + the new admin targets if Suize wants to sponsor/automate rate changes.
